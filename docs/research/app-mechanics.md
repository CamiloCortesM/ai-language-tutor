...
**Replicate simply:** a voice-mode LLM (the voice bridge to ChatGPT/Claude/any voice AI) **hears the learner's actual audio** — it runs repeat-after-me and minimal-pair drills live, catches mispronounced words and off intonation, and coaches articulation ("your /θ/ in 'think' comes out as /s/ — put your tongue between your teeth"). That covers most of ELSA's value with zero infra. What it doesn't give: ELSA's calibrated per-phoneme color scores — for that, a real scoring model (Azure Speech "pronunciation assessment" API, or `wav2vec2`). Text-only-harness fallback: record → **STT with word timestamps** (whisper.cpp or API) → compare transcription against the target text (catches gross errors, not nuances).
